...
type ProductosReports struct {
	ReportsUseCase reports.UseCase
}

func (handler ProductosReports) Handle(ginContext *gin.Context) {
	handler.handle(ginContext)
}

func (handler ProductosReports) handle(ctx *gin.Context) {
	...
	result, err := handler.ReportsUseCase.GetStockProductosDesactivados()
	...
	ctx.JSON(http.StatusOK, contracts.NewResponseStockProductosDesactivados(result))
	return
}
